...
public class Scanner {
    ...
    private void number() {
        ...
        if(peek() == '.' && isDigit(peekNext())) {
            ...
            while(isDigit(peek())) { advance(); } // Consume the number chars after the decimal point
        }
    ...
    private void string() {
        ...
        String value = source.substring(start + 1, current - 1); // Trim off the quotes on either side
        addToken(STRING, value);
    }

    private boolean match(char expected) {
        if (isAtEnd()) { return false; }
        if (source.charAt(current) != expected) { return false; } // Don't consume if it doesn't match
